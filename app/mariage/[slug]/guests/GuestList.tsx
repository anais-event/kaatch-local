...
export default function GuestList({
  ...
  // Build family groups for family view
  const familyGroups = (() => {
    if (viewMode !== 'family') return []
    const map = new Map<string, Guest[]>()
    for (const g of filtered) {
      const key = g.family_name || '__sans_famille__'
      if (!map.has(key)) map.set(key, [])
      map.get(key)!.push(g)
    }
    // Sort: named families first (alphabetical), then sans famille
    const entries = Array.from(map.entries()).sort((a, b) => {
      if (a[0] === '__sans_famille__') return 1
      if (b[0] === '__sans_famille__') return -1
  ...
  function renderGuestRow(guest: Guest) {
    ...
    return (
      <div key={guest.id} className={isSelected ? 'bg-[#4a5240]/[0.03]' : ''}>
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0 cursor-pointer"
            ...
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              ...
              {guest.dietary_notes && (
                <span title={guest.dietary_notes} className="text-orange-400 text-[10px] leading-none" style={{ fontWeight: 300 }}>
          ...
          <div onClick={e => e.stopPropagation()} className="shrink-0 hidden sm:block">
            ...
            {savingNotes === guest.id && <span className="absolute text-[10px] text-stone-300 mt-0.5 ml-1">✓</span>}
          </div>
          {link && (
          ...
          <div className="px-5 pb-5 pt-3 border-t border-stone-50 bg-stone-50/30">
            {(guest.email || guest.telephone) && (
              <div className="flex flex-wrap gap-3 mb-3">
                {guest.email && (
                  <a href={`mailto:${guest.email}`} className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-[#4a5240] transition" style={{ fontWeight: 300 }}>
                ...
                {guest.telephone && (
                  <a href={`tel:${guest.telephone}`} className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-[#4a5240] transition" style={{ fontWeight: 300 }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-3.5 h-3.5 text-stone-300">
            ...
            {guest.guest_message && (
              <div className="mb-3 bg-amber-50/60 rounded-xl px-3 py-2">
                <p className="text-xs text-amber-700 italic" style={{ fontWeight: 300 }}>"{guest.guest_message}"</p>
              </div>
            )}
        ...
        {/* ── Formulaire édition ── */}
        {isEditing && (
          <form action={async (fd) => { startTransition(async () => { await updateGuest(fd); setEditingId(null) }) }}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="flex gap-1 bg-stone-100/60 p-1 rounded-xl w-fit">
          {FILTER_TABS.map(f => (
        ...
        <button
          onClick={() => setViewMode(v => v === 'list' ? 'family' : 'list')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs transition cursor-pointer border ${
            viewMode === 'family'
              ? 'bg-[#4a5240]/10 text-[#4a5240] border-[#4a5240]/20'
              : 'bg-white text-stone-400 border-stone-200 hover:text-stone-600'
          }`}
          style={{ fontWeight: viewMode === 'family' ? 500 : 300 }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-3.5 h-3.5">
        ...
        {(typeCountsInFiltered.enfant > 0 || typeCountsInFiltered.ado > 0 || typeCountsInFiltered.animal > 0) && (
          <div className="flex items-center gap-2 text-xs text-stone-400 ml-auto" style={{ fontWeight: 300 }}>
            ...
            {typeCountsInFiltered.animal > 0 && (
              <span>🐶 {typeCountsInFiltered.animal}</span>
            )}
          </div>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
            ...
            return (
              <div key={familyKey} className="bg-white rounded-2xl border border-stone-100 overflow-hidden">
                ...
                <button
                  ...
                  <span style={{ fontWeight: isSansFamille ? 300 : 500, fontSize: '0.85rem' }}
                        className={isSansFamille ? 'text-stone-400 italic' : 'text-[#2d3228]'}>
                    {familyLabel}
                  </span>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 overflow-hidden">
          ...
          <div className="divide-y divide-stone-50">
            {filtered.map(guest => renderGuestRow(guest))}
          </div>
        </div>
      )}
