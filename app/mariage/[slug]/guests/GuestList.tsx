...
function Avatar({ name, gender }: { name: string; gender: 'M' | 'F' | null }) {
  ...
  return (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${color}`}
         style={{ fontWeight: 500, fontSize: '0.72rem', letterSpacing: '0.02em' }}>
      {initials}
    </div>
  )
}
          ...
          {filtered.map(guest => {
            ...
            return (
              <div key={guest.id}>
                ...
                {/* ── Ligne principale ── */}
                <div
                  className="flex items-center gap-3.5 px-5 py-4 cursor-pointer hover:bg-stone-50/50 transition"
                  ...
                  {/* Chevron */}
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}
                       className={`w-4 h-4 text-stone-200 shrink-0 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}>
